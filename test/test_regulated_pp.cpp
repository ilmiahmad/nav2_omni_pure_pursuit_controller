...
class BasicAPIRPP : public nav2_pure_pursuit_controller::PurePursuitController
{
public:
  BasicAPIRPP()
...
TEST(PurePursuitTest, createCarrotMsg)
{
  auto ctrl = std::make_shared<BasicAPIRPP>();
...
TEST(PurePursuitTest, findVelocitySignChange)
{
  auto ctrl = std::make_shared<BasicAPIRPP>();
...
TEST(PurePursuitTest, lookaheadAPI)
{
  auto ctrl = std::make_shared<BasicAPIRPP>();
  auto node = std::make_shared<rclcpp_lifecycle::LifecycleNode>("testRPP");
  std::string name = "PathFollower";
...
TEST(PurePursuitTest, rotateTests)
{
  auto ctrl = std::make_shared<BasicAPIRPP>();
  auto node = std::make_shared<rclcpp_lifecycle::LifecycleNode>("testRPP");
  std::string name = "PathFollower";
...
TEST(PurePursuitTest, applyConstraints)
{
  auto ctrl = std::make_shared<BasicAPIRPP>();
  auto node = std::make_shared<rclcpp_lifecycle::LifecycleNode>("testRPP");
  std::string name = "PathFollower";
...
TEST(PurePursuitTest, testDynamicParameter)
{
  ...
  costmap->on_configure(rclcpp_lifecycle::State());
  auto ctrl =
    std::make_unique<nav2_pure_pursuit_controller::PurePursuitController>();
  auto tf = std::make_shared<tf2_ros::Buffer>(node->get_clock());
